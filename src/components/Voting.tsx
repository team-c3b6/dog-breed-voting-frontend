...
export default function Voting(): JSX.Element {
  ...
  const handleVote = async (num: number) => {
    await axios.post(apiBaseURL + "/breeds", {
  ...
  return (
  <>
    <img src={options[0]} alt={retrieveBreedName(options[0])} onClick={() => handleVote(0)} width="300"/>
    <p>{changeName(retrieveBreedName(options[0]))}</p>
    <img src={options[1]} alt={retrieveBreedName(options[0])}onClick={() => handleVote(1)} width="300"/>
